!!! warning "تنبيه"
    إنذار : تم ترجمة هذه الصفحة باستخدام الذكاء الاصطناعي.

# الفصل الرابع: نظرية الأعداد والتشفير

هذه الصفحة تحتوي على مادة الفصل الرابع. استخدمها للمراجعة عن الأعداد الصحيحة، قابلية القسمة، الحساب النمطي (modular arithmetic)، والمفاهيم الأساسية في التشفير.

إذا ما تحمل العرض بشكل صحيح، افتح الفصل بشكل مباشر.

- [افتح الفصل الرابع في صفحة جديدة](../Number-Theory-and-Cryptography.html)

<div class="iframe-wrap">
  <iframe
    src="../Number-Theory-and-Cryptography.html"
    title="الفصل الرابع: نظرية الأعداد والتشفير"
    loading="lazy"
    referrerpolicy="no-referrer"
  ></iframe>
</div>