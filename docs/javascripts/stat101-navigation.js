(function () {
  ...
  function normalizeCourseNavigation() {
    var sectionList = document.querySelector(
      ".academic-sidebar .section-children",
    );
    if (!sectionList) return;

    var courseMatch = window.location.pathname.match(
      /^\/academics\/(?:other-courses|math)\/([^/]+)\//i,
    );
    if (!courseMatch) return;
    var isMathCourse =
      window.location.pathname.indexOf("/academics/math/") === 0;
    var base =
      "/academics/" +
      (isMathCourse ? "math" : "other-courses") +
      "/" +
      courseMatch[1].toLowerCase() +
      "/";
    var order = [
      base,
    ...
    if (base === "/academics/other-courses/math221/") {
      nestedLists.forEach(function (list) {
        ...
        items.sort(function (left, right) {
          ...
          for (
            var index = 0;
            index < Math.max(leftNumbers.length, rightNumbers.length);
            index += 1
          ) {
            var difference =
              (leftNumbers[index] == null ? -1 : leftNumbers[index]) -
              (rightNumbers[index] == null ? -1 : rightNumbers[index]);
    ...
    var worksheetItems = Array.from(studyList.children).filter(function (item) {
      ...
      link.setAttribute(
        "href",
        base + "extra-resources/worksheets/" + slug + "/",
      );
      return true;
    });
    ...
    var inWorksheets =
      window.location.pathname.indexOf(base + "extra-resources/worksheets/") ===
      0;
    var folderItem = document.createElement("li");
    folderItem.className =
      "tree-item tree-viewer" + (inWorksheets ? " file-active" : "");
    folderItem.innerHTML =
      '<a class="tree-file" href="' +
      base +
      'extra-resources/worksheets/">' +
      (inWorksheets ? '<span class="status-dot"></span>' : "") +
      "WORKSHEETS/</a>";
    studyList.insertBefore(folderItem, worksheetItems[0]);

    var worksheetList = document.createElement("ul");
    worksheetList.className =
      "tree-children item-children folder-children" +
      (inWorksheets ? " is-open" : "");
