(function () {
  ...
  var endpoint =
    window.SHOUG_CALENDAR_AI_ENDPOINT ||
    "https://shoug-tech.shoug-alomran.workers.dev/v1/calendar-assistant";
  var history = [];
  var MODES = {
    chat: {
      tab: "Ask",
      intent: "chat",
      placeholder: "What do I have this week?",
      hint: "Answers come from the classes, exams and events on your calendar.",
    },
    add: {
      tab: "Add",
      intent: "parse",
      placeholder:
        "SE322 final on 2026-05-12 from 14:00 to 16:00 in building 5",
      hint: "Describe it in a sentence. Nothing is saved until you press Add.",
    },
    plan: {
      tab: "Plan",
      intent: "plan",
      placeholder: "Plan revision for my SE322 final",
      hint: "Proposes study sessions around your existing classes. Nothing is saved until you press Add.",
    },
    brief: {
      tab: "Brief",
      intent: "brief",
      placeholder: "Pick an exam below, then ask for what to focus on.",
      hint: "Builds a revision brief from that course's material on this site.",
    },
  };
  var mode = "chat";
  ...
  function localPlanRevision(question) {
    ...
    if (
      !/(from scratch|haven.?t (started|touched)|beginner|no prior|shorter|fewer|evening|after class)/i.test(
        lower,
      )
    )
      return null;
    ...
    var bridge = api(),
      course =
        revised[0].course ||
        revised[0].title.split(/\s+(?:study|review|chapter|quiz)/i)[0];
    var exam =
      bridge &&
      bridge.upcomingExams().find(function (entry) {
        return (
          String(entry.course || "")
            .replace(/[^a-z0-9]/gi, "")
            .toLowerCase() ===
    ...
    if (exam)
      revised = revised.filter(function (proposal) {
        return proposal.date < exam.date;
      });
    if (/fewer/i.test(lower)) revised = revised.slice(0, 3);
    if (
      /from scratch|haven.?t (started|touched)|beginner|no prior/i.test(lower)
    ) {
      var source = lastPlanProposals
        .map(function (proposal) {
          return [proposal.title, proposal.note].join(" ");
        })
        .join(" ");
      var range = source.match(/chapters?\s*(\d+)\s*(?:-|–|to)\s*(\d+)/i),
        chapters = [];
      if (range)
        for (
          var number = Number(range[1]);
          number <= Number(range[2]) && chapters.length < 8;
          number++
        )
      ...
      revised = revised.map(function (proposal, index) {
        var chapter =
          chapters[
            Math.min(
              chapters.length - 1,
              Math.floor(
                (index * chapters.length) / Math.max(1, revised.length),
              ),
            )
          ];
        ...
        return Object.assign({}, proposal, {
          course: course,
          title: finalStep
            ? course + " Mixed Practice & Recall"
            : course + " Learn Chapter " + chapter,
          note: finalStep
            ? "Test yourself across all chapters, then revisit mistakes"
            : "Start from the basics: learn Chapter " +
              chapter +
              ", make notes, then use active recall",
        });
      });
    }
    revised = revised.map(function (proposal) {
      var start = timeMinutes(proposal.start),
        duration = Math.max(
          45,
          (timeMinutes(proposal.end) || (start === null ? 0 : start + 90)) -
            (start === null ? 0 : start),
        );
  ...
  function api() {
    return window.__shougCalendarAI || null;
  }
  ...
  function fillExams() {
    ...
    if (!exams.length) {
      examSelect.innerHTML =
        '<option value="">No upcoming exams on your calendar</option>';
      return;
    }
  ...
  function describe(proposal) {
    var when = proposal.start
      ? proposal.start + (proposal.end ? "–" + proposal.end : "")
      : "all day";
    var span =
      proposal.endDate && proposal.endDate !== proposal.date
        ? proposal.date + " → " + proposal.endDate
        : proposal.date;
  ...
  function card(proposal) {
    ...
    title.textContent =
      proposal.kind === "exam" && proposal.course
        ? proposal.course + " " + (proposal.examType || "exam")
        : proposal.title;
    ...
    add.addEventListener("click", function () {
      ...
      var save =
        proposal.kind === "exam"
          ? bridge.addExam(proposal)
          : bridge.addEvent(proposal);
  ...
  panel.querySelector(".cal-ai-close").addEventListener("click", function () {
    setOpen(false);
  });
  ...
  panel.querySelector("form").addEventListener("submit", function (event) {
    ...
    if (!bridge || !bridge.signedIn()) {
      say(
        "bot err",
        "Sign in to use the calendar assistant — it works from your saved classes and exams.",
      );
      return;
    }
    ...
    if (mode === "plan") {
      ...
      if (localRevision) {
        ...
        say(
          "bot",
          "I revised the previous plan for " +
            (localRevision[0].course || "that exam") +
    ...
    if (mode === "brief") {
      ...
      if (!selection) {
        say(
          "bot err",
          "Add an upcoming exam to your calendar first, then I can build a brief for it.",
        );
        return;
      }
      ...
      if (!payload.course) {
        say(
          "bot err",
          "I could not match " + parts[0] + " to a course page on this site.",
        );
        return;
      }
    }
    ...
    var pending = say(
      "bot",
      mode === "plan" ? "Looking for free slots…" : "Working…",
    );
    ...
    fetch(endpoint, {
      ...
      .then(function (data) {
        ...
        if (data.proposals && data.proposals.length) {
          say(
            ...
            data.proposals.length === 1
              ? "Here is what I understood — nothing is saved yet:"
              : "Here is what I propose — nothing is saved yet:",
          );
        ...
        var node = say(
          "bot",
          data.answer || "I could not work that out from your calendar.",
        );
  ...
  say(
    "bot",
    "Ask about your schedule, add something in plain words, plan revision around your classes, or build a brief for an upcoming exam.",
  );
})();
