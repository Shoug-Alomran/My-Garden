(function () {
  ...
  try {
    desired = desired || localStorage.getItem("shoug-breakdown-lang");
  } catch (_) {}
  ...
  function start() {
    ...
    var endpoint =
      window.SHOUG_TRANSLATION_ENDPOINT ||
      "https://shoug-tech.shoug-alomran.workers.dev/v1/breakdown-translation";
    var records = new Map();
    ...
    toolbar.innerHTML =
      '<div><button type="button" data-bd-lang="en" lang="en" aria-label="English">EN</button><button type="button" data-bd-lang="ar" lang="ar" aria-label="العربية">AR</button></div><span class="bd-language-status" role="status" aria-live="polite"></span>';
    var host = document.querySelector(
      ".bdx-bar-inner, .topbar-actions, .header-actions, .topbar-inner",
    );
    if (host) {
      ...
      if (themeButton)
        themeButton.parentNode.insertBefore(toolbar, themeButton);
      else host.appendChild(toolbar);
    } else {
      toolbar.classList.add("bd-language--inline");
      (
        document.querySelector(
          "#main-content, main, .header, .topbar, header",
        ) || document.body
      ).prepend(toolbar);
    }
    ...
    var skip =
      'script,style,noscript,code,pre,kbd,samp,svg,math,textarea,input,[contenteditable],.bd-language,.sg-ai-panel,.sg-ai-launch,.sys-time,[translate="no"],#arabicContent,#shoug-fb-user,#shoug-auth-modal,#shoug-ue-modal,#shoug-page-comments,#shoug-notes-panel,#shoug-ob-card,.shoug-user-dropdown';
    ...
    document
      .querySelectorAll("#langToggle,[data-lang-toggle],.shoug-lang-btn")
      .forEach(function (button) {
        button.hidden = true;
      });
    document
      .querySelectorAll('[data-lang-panel="ar"]')
      .forEach(function (panel) {
        var other = panel.parentElement.querySelector(
          '[data-lang-panel="en"] iframe',
        );
    ...
    function collect() {
      ...
      document.body
        ...
        .forEach(function (element) {
          ...
          ["placeholder", "title", "aria-label", "alt"].forEach(
            function (attribute) {
    ...
    function render(lang) {
      ...
      toolbar.querySelectorAll("button").forEach(function (button) {
        button.setAttribute(
          "aria-pressed",
          String(button.dataset.bdLang === lang),
        );
      });
    }
      ...
      document.querySelectorAll("iframe").forEach(function (frame) {
        try {
          if (
            ...
            new URL(frame.src, location.href).origin === location.origin
          )
            frame.contentWindow.postMessage(
              { type: "shoug-breakdown-language", lang: lang },
              location.origin,
            );
        } catch (_) {}
      });
      ...
      try {
        ...
        try {
          Object.keys(localStorage).forEach(function (key) {
            if (key.indexOf("bd-ar-") === 0 && key !== storeKey)
    ...
    async function fetchBatch(texts, signal) {
      ...
      return data.translations.map(function (text, i) {
        if (validTranslation(text, texts[i])) {
          stored[texts[i]] = text;
          return text;
        }
        return texts[i];
    ...
    async function setLanguage(lang, persist) {
      ...
      if (persist) {
        try {
          localStorage.setItem("shoug-breakdown-lang", desired);
        } catch (_) {}
      ...
      try {
        ...
        records.forEach(function (slots) {
          slots.forEach(function (record) {
            pieces(record.en).forEach(function (part) {
              if (!/[A-Za-z]{3}/.test(part) || isReference(part))
                translations.set(part, part);
        ...
        missing.forEach(function (text) {
          if (typeof stored[text] === "string") {
            translations.set(text, stored[text]);
            missing.delete(text);
          }
        });
        var batches = [],
        ...
        missing.forEach(function (text) {
          if (
            batch.length &&
            (batch.length >= 16 || size + text.length > 2500)
          ) {
            batches.push(batch);
            batch = [];
            size = 0;
            ...
            try {
              ...
              try {
                ...
                await new Promise(function (resolve) {
                  setTimeout(resolve, first.status === 429 ? 20000 : 1500);
                });
                if (version !== revision) return;
                values = await fetchBatch(texts, signal);
              }
              ...
              texts.forEach(function (text, i) {
                translations.set(text, values[i]);
              });
              done++;
              apply();
        ...
        var workers = [];
        for (var w = 0; w < Math.min(4, batches.length); w++)
          workers.push(worker());
        ...
        if (failure)
          report(
            "Part of this lesson is still in English. Select AR to finish. / جزء من الشرح لم يُترجم بعد، اضغط AR لإكماله.",
            true,
          );
    ...
    window.addEventListener("message", function (event) {
      if (
        ...
        !event.data ||
        event.data.type !== "shoug-breakdown-language"
      )
        return;
      var trusted =
      ...
      if (
        !trusted ||
        (event.data.lang === desired && (busy || active === desired))
      )
        return;
      if (event.data.lang === "ar" || event.data.lang === "en")
        setLanguage(event.data.lang, false);
    });
    window.addEventListener("storage", function (event) {
      if (
        event.key === "shoug-breakdown-lang" &&
    ...
    window.__shougSetLanguage = function (lang) {
