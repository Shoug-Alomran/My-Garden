...
for (const htmlFile of htmlFiles) {
  ...
  const relativeViewer = path
    .relative(docs, viewerDirectory)
    .split(path.sep)
    .join("/");
  if (!/\/(slides|extra-resources|exams)\/.+/.test(`/${relativeViewer}`))
    continue;

  const contents = fs.readFileSync(htmlFile, "utf8");
  const pdfUrls = [
    ...contents.matchAll(
      /(?:href|src)="(\/academics\/other-courses\/(?:math221|com201)\/[^"?#]+\.pdf)"/gi,
    ),
  ].map((match) => decodeURI(match[1]));

  for (const pdfUrl of new Set(pdfUrls)) {
    const source = path.join(docs, pdfUrl.replace(/^\//, ""));
    if (!fs.existsSync(source) || path.dirname(source) === viewerDirectory)
      continue;
...
const moves = [...movesBySource.entries()].map(([source, target]) => ({
  source,
  target,
}));
...
console.log(
  `Moved ${moves.length} PDFs into their viewer folders and updated ${htmlFiles.length} HTML files.`,
);
for (const { source, target } of moves) {
  console.log(
