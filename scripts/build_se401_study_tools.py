...
EXAMS = BASE / "exams"
# Optional {chapter_no: {"mcq": [...], "short": [...]}} of hand-written questions.
QUIZ_BANK = None
...
def exam_html(chapter_no: int, title: str, branches: list) -> str:
    ...
    selected = facts
    mcqs = []
    bank = (QUIZ_BANK or {}).get(chapter_no)
    if bank:
        # Hand-written questions replace the generated topic-matching items,
        # whose options were the same chapter topic names on every question.
        for q, correct, wrong, why in bank["mcq"]:
            opts = [correct, *wrong]
            random.Random(hashlib.sha1(q.encode()).hexdigest()).shuffle(opts)
            mcqs.append({"q": html.escape(q, quote=False), "options": [html.escape(o, quote=False) for o in opts], "correct": opts.index(correct), "why": html.escape(why, quote=False)})
        selected = []
    ...
    shorts = [{"q": html.escape(q, quote=False), "keywords": kw, "answer": html.escape(a, quote=False)} for q, kw, a in bank["short"]] if bank else []
    for raw_branch in ([] if bank else branches):
