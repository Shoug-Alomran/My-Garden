...
DESKTOP = "(() => {" + HELPERS + r"""
  ...
  for (const a of mainChain(blocks)) {
    ...
    // Computed tracks are always px, even for 1fr. Flexible tracks fill the grid's
    // content box; only a grid with space left beside its tracks has a fixed column.
    const used = tracks.reduce((n, t) => n + parseFloat(t), 0) + (parseFloat(pcs.columnGap) || 0) * (tracks.length - 1);
    const box = parent.clientWidth - parseFloat(pcs.paddingLeft) - parseFloat(pcs.paddingRight);
    if (used >= box - 4) continue;
    const sel = selectorOf(parent);
    if (seenGrid.has(sel)) continue;
    seenGrid.add(sel);
    gridWiden.push([sel, tracks.slice(0, -1).join(' '), tracks[tracks.length - 1]]);
  }
...
def css_for(found: dict) -> str:
    ...
    # Grid tracks were measured at 1440px, so only apply them on wide screens: pinned
    # pixel columns would override a page's own single-column phone layout. Equal
    # columns (a 3-up card grid) stay equal and flexible instead of frozen in pixels.
    wide = {}
    for selector, prefix, *last in sorted(found["gridwiden"]):
        # compare every track, the last included: a lone "224px" rail before a 900px
        # column is not an equal-column grid and must keep its fixed width
        all_tracks = prefix.split() + last
        widths = [float(t[:-2]) for t in all_tracks if t.endswith("px")]
        if len(widths) == len(all_tracks) and len(widths) > 1 and max(widths) - min(widths) < 2:
            tracks = f"repeat({len(widths)}, minmax(0, 1fr))"
        else:
            tracks = f"{prefix} minmax(0, 1fr)" if prefix else "minmax(0, 1fr)"
        wide.setdefault(selector, tracks)
    if wide:
        rules = "\n\n  ".join(f"{sel}{BUMP} {{\n    max-width: min(1320px, 100%);\n    grid-template-columns: {tracks};\n  }}"
                              for sel, tracks in wide.items())
        parts.append(f"@media (min-width: 1100px) {{\n  {rules}\n}}")
    if found["release"]:
