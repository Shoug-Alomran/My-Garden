...
for (const [section, lessons] of Object.entries(sections)) {
  ...
  for (const [oldFolder, newFolder, newPdf, label] of lessons) {
    ...
    const workingDirectory = fs.existsSync(oldDirectory)
      ? oldDirectory
      : newDirectory;
    const currentPdf = fs
    ...
    if (currentPdf && currentPdf !== newPdf) {
      fs.renameSync(
        path.join(workingDirectory, currentPdf),
        path.join(workingDirectory, newPdf),
      );
    }
    if (oldDirectory !== newDirectory) {
      if (fs.existsSync(oldDirectory) && fs.existsSync(newDirectory))
        throw new Error(`Target folder exists: ${newDirectory}`);
...
for (const htmlFile of htmlFiles) {
  ...
  for (const [oldFolder, newFolder] of Object.entries(staleSlideRoutes)) {
    contents = contents
      .split(`/academics/other-courses/math221/slides/${oldFolder}/`)
      .join(`/academics/other-courses/math221/slides/${newFolder}/`);
  }
  for (const item of replacements) {
    const escapedRoute = item.newRoute.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const rowPattern = new RegExp(
      `(<a href="${escapedRoute}" class="dir-row"[\\s\\S]*?<div class="dir-title">)[\\s\\S]*?(<\\/div>)`,
      "g",
    );
    contents = contents.replace(rowPattern, `$1${item.label}$2`);
    const treePattern = new RegExp(
      `(<a class="tree-file" href="${escapedRoute}">)(?:<span class="status-dot"><\\/span>)?[^<]*(<\\/a>)`,
      "g",
    );
    contents = contents.replace(
      treePattern,
      (match, start, end) =>
        `${start}${match.includes("status-dot") ? '<span class="status-dot"></span>' : ""}${item.label}${end}`,
    );
  }
...
for (const item of replacements) {
  ...
  contents = contents.replace(
    /<title>.*?<\/title>/,
    `<title>MATH221 | ${item.label}</title>`,
  );
  contents = contents.replace(
  ...
  contents = contents.replace(
    ...
    `<h1 class="ch-title">${item.label}</h1>`,
  );
  contents = contents.replace(
    /(<iframe class="embed-frame"[^>]* title=")[^"]*(")/,
    `$1${item.label}$2`,
  );
...
for (const section of ["slides", "extra-resources"]) {
  ...
  const rows = [
    ...container.matchAll(
      /<a href="[^"]+" class="dir-row"[^>]*>[\s\S]*?<\/a>/g,
    ),
  ].map((match) => match[0]);
  if (!rows.length) continue;
  rows.sort((left, right) => {
    const leftLabel =
      left.match(/<div class="dir-title">([\s\S]*?)<\/div>/)?.[1].trim() || "";
    const rightLabel =
      right.match(/<div class="dir-title">([\s\S]*?)<\/div>/)?.[1].trim() || "";
    return compareLessons(leftLabel, rightLabel);
  });
  const numberedRows = rows.map((row, index) =>
    row.replace(
      /<div class="dir-num">.*?<\/div>/,
      `<div class="dir-num">${String(index + 1).padStart(2, "0")}</div>`,
    ),
  );
  const firstRow = container.search(/<a href="[^"]+" class="dir-row"/);
  const lastRowEnd = container.lastIndexOf("</a>") + 4;
  const sortedContainer =
    container.slice(0, firstRow) +
    numberedRows.join("\n                            ") +
    container.slice(lastRowEnd);
  contents =
    contents.slice(0, containerStart) +
    sortedContainer +
    contents.slice(rightsStart);
  fs.writeFileSync(indexFile, contents);
}

console.log(
  `Simplified ${replacements.length} MATH221 lesson folders and their PDFs.`,
);
