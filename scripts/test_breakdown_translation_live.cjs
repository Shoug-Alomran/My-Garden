...
assert.ok(
  url.pathname.includes("/slide-breakdowns/") && url.pathname.endsWith(".html"),
);
async function main() {
  ...
  try {
    ...
    if (process.env.BREAKDOWN_TEST_LOCAL_ASSETS === "1") {
      ...
      for (const [asset, contentType] of [
        ["javascripts/breakdown-language.js", "text/javascript"],
        ["styles/breakdown-language.css", "text/css"],
      ]) {
    ...
    page.on("response", (response) => {
      ...
      if (!response.ok()) {
        ...
        if (process.env.BREAKDOWN_FAILURE_PATH)
          require("node:fs").writeFileSync(
            process.env.BREAKDOWN_FAILURE_PATH,
    ...
    page.on("requestfailed", (request) => {
      if (request.url().includes("/v1/breakdown-translation"))
    ...
    if (originalHeading.includes("4+1"))
      assert.ok(
        arabicHeading.includes("4+1"),
        "The title must keep the 4+1 model identity",
      );
    ...
    assert.ok(
      counts.arabic > counts.latin,
      "Lesson prose must actually become Arabic",
    );
    assert.deepEqual(failures, []);
    if (process.env.BREAKDOWN_SCREENSHOT)
      await page.screenshot({ path: process.env.BREAKDOWN_SCREENSHOT });
    ...
    console.log(
      JSON.stringify({
        ...
        url: url.href,
        arabicHeading,
        localAssets: process.env.BREAKDOWN_TEST_LOCAL_ASSETS === "1",
