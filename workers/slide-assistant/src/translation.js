...
function decodeTranslation(raw, originals) {
  ...
  const values = Array.isArray(parsed) ? parsed : originals.map((_, index) => parsed && parsed['item_' + String(index).padStart(3, '0')]);
...
function untranslatedIndices(values, originals) {
  return originals.flatMap((original, index) => {
    ...
    const expressions = original.replace(/\s+/g, '').match(/\d+(?:[+*/]\d+)+/g) || [];
    const invalid = typeof value !== 'string' || !value.trim() || expressions.some(expression => !value.replace(/\s+/g, '').includes(expression)) ||
...
export async function translateRequest(request, env, origin, json, generate) {
  ...
  const cacheKey = new Request(new URL('/translation-cache/v5/' + hash, request.url));
  ...
  async function run(texts, repair = false) {
    const entries = Object.fromEntries(texts.map((text, index) => ['item_' + String(index).padStart(3, '0'), text.replace(/\s+/g, ' ').trim()]));
    const properties = Object.fromEntries(Object.keys(entries).map(key => [key, { type: 'string' }]));
    ...
    return generate(env, {
      messages: [
        { role: 'system', content: instruction + (repair ? ' These specific entries were previously missed or left in English. Every returned value MUST now include its Arabic translation or Arabic transcription, including named frameworks. Do not just copy the English input.' : '') },
        { role: 'user', content: JSON.stringify(entries) }
      ],
      response_format: {
        type: 'json_schema',
        json_schema: {
          name: 'arabic_translation', strict: true,
          schema: { type: 'object', properties, required: Object.keys(entries), additionalProperties: false }
        }
      },
