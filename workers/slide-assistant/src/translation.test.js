...
test("validates bounded non-empty text batches", () => {
  assert.equal(
    validateTexts(["Information security", "Risk assessment"]),
    true,
  );
  for (const texts of [
    [],
    ...
    assert.equal(validateTexts(texts), false);
});
test("rejects missing, untranslated, or malformed model output", () => {
  assert.deepEqual(
    parseTranslation('["أمن المعلومات"]', ["Information security"]),
    ["أمن المعلومات"],
  );
  assert.deepEqual(parseTranslation('["CIA","TCP/IP"]', ["CIA", "TCP/IP"]), [
    "CIA",
    "TCP/IP",
  ]);
  assert.deepEqual(
    parseTranslation(
      '["https://example.com","student@example.com","guide.pdf"]',
      ["https://example.com", "student@example.com", "guide.pdf"],
    ),
    ["https://example.com", "student@example.com", "guide.pdf"],
  );
  for (const text of ["[]", '["Information security"]', "[null]", "not json"])
    assert.throws(() => parseTranslation(text, ["Information security"]));
});
test("translation route returns Arabic with site CORS", async () => {
  const response = await worker.fetch(
    request({ texts: ["Information security"] }),
    {
      AI: {
...
test("bad input, untrusted origins, rate limits and incomplete output do not succeed", async () => {
  ...
  assert.equal(
    (
      await worker.fetch(
        request({ texts: ["hello"] }, "https://untrusted.test"),
        {},
      )
    ).status,
    403,
  ...
  assert.equal(
    (
      await worker.fetch(request({ texts: ["hello"] }), {
        TRANSLATION_RATE_LIMIT: { limit: async () => ({ success: false }) },
      })
    ).status,
    429,
  );
...
test("repairs only untranslated titles, names and mnemonics before returning a complete batch", async () => {
    ...
    }),
    {
      AI: {
        run: async (_model, input) => {
          ...
          assert.equal(
            input.response_format.json_schema.schema.required.length,
            texts.length,
          );
          if (calls === 1) {
  ...
  assert.deepEqual((await response.json()).translations, [
    "أهداف التعلم",
    "نموذج الرؤى 4+1",
    "كروشتن، 1995",
    "الأسود تفضل البيتزا الداكنة",
...
test("keyed model output keeps fragmented sentences aligned even if key order changes", () => {
  assert.deepEqual(
    parseTranslation(
      '{"item_002":"مع بعضها","item_000":"حافظ على","item_001":"الاتساق"}',
      ["keep", "consistent", "with each other"],
    ),
    ["حافظ على", "الاتساق", "مع بعضها"],
  );
  assert.throws(() =>
    parseTranslation('{"item_000":"حافظ على الاتساق"}', ["keep", "consistent"]),
  );
});

test("keeps mathematical expressions intact", () => {
  assert.throws(() =>
    parseTranslation('["نموذج الرؤى +1-4"]', ["The 4+1 View Model"]),
  );
  assert.deepEqual(
    parseTranslation('["نموذج الرؤى 4+1"]', ["The 4+1 View Model"]),
    ["نموذج الرؤى 4+1"],
  );
});
